...
ggplot(data.frame(x=assay(petro_DB$sce_dd)["P46061",], covariate = colData(petro_DB$sce_dd)[["SampleType"]])) +
  stat_ecdf(aes(x=x, colour = covariate)) +
  theme_paper() + scale_color_paper() -> DB_distinct

visFit(petro_DB$sce_dd, feature = "P46061", plot = "hist")$SampleTypeA_VS_SampleTypeB +
