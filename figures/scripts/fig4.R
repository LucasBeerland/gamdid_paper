...
plot_GAM_DE_leduc <- plotFdp(leduc_DE$fdp, selection = c("GAM", "msqrob2", "distinct"), title = "DE") +
  theme_paper() +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = base_size, face = "bold", hjust = 0.5)) +
  scale_color_simulations()
plot_GAM_DV_leduc <- plotFdp(leduc_DV$fdp, selection = c("GAM", "msqrob2", "distinct"), title = "DV") +
  theme_paper() +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = base_size, face = "bold", hjust = 0.5)) +
  scale_color_simulations()
plot_GAM_DB_leduc <- plotFdp(leduc_DB$fdp, selection = c("GAM", "msqrob2", "distinct"), title = "DB") +
  theme_paper() +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = base_size, face = "bold", hjust = 0.5)) +
  scale_color_simulations()
plot_GAM_DM_leduc <- plotFdp(leduc_DM$fdp, selection = c("GAM", "msqrob2", "distinct"), title = "DM") +
  theme_paper() +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = base_size, face = "bold", hjust = 0.5)) +
  scale_color_simulations()
plot_GAM_DP_leduc <- plotFdp(leduc_DP$fdp, selection = c("GAM", "msqrob2", "distinct"), title = "DP") +
  theme_paper() +
  theme(plot.title = element_text(size = base_size, face = "bold", hjust = 0.5)) +
  scale_color_simulations()

ggarrange(plot_GAM_DE_leduc, plot_GAM_DV_leduc, plot_GAM_DB_leduc, plot_GAM_DM_leduc, plot_GAM_DP_leduc,
          common.legend = T, ncol = 1, legend = "top", heights = c(1,1,1,1,1.3)) -> fig4
